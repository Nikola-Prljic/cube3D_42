...
/* void draw_texture_to_wall(t_data *data, t_rect rect)
{
    int x;
    int end_y;
    int end_x;
    ...
    while(rect.start_y < end_y)
    {
        ...
        while(x < end_x)
        {
            img_pix_put(&data->img, (t_rect){(double)WINDOW_HEIGT / 2.0 - wallStripHeight / 2.0, data->raycount, wallStripHeight, 1, img->addr[(y_color * img->line_len) + x_color]});
        }
        rect.start_y++;
    }
} */

/* void put_texture_y(t_data *data, t_xpm_img *img)
{
